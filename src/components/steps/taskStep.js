...
const TubeProgressBar = ({ excuted_tubes }) => {
  ...
  useEffect(() => {
    setCurrentSteps(excuted_tubes.map(task => task.currentTubeId || undefined));
  }, [excuted_tubes]);
  ...
  return (
    <div className='step-content'>
      {excuted_tubes.map((task, taskIndex) => {
        ...
        return(
            <div>  <div style={{textAlign:"left" ,marginTop:"-12px",zIndex: 99,position: "absolute",
            }}>{statusText}</div>
        <div key={taskIndex} style={{ display: 'flex', flexWrap: 'wrap' ,marginTop:"1rem"}}>
           
          {task.tube_list.map((tubeId) =>{
            const stepStyle = getStepColor(taskIndex, tubeId);
            return (
            <div
            className={
                typeof stepStyle === 'object'? stepStyle.className : "tube-list-container"
              }
                key={`${taskIndex}-${tubeId}`}
                style={{
                    backgroundColor: typeof stepStyle === 'object'? stepStyle.backgroundColor : stepStyle,
                    color: getStepTextColor(taskIndex, tubeId),
                    borderColor: getStepBorderColor(taskIndex, tubeId)
              }}
            >
